...
app.get('/recommendRestaurant/:userName', async (req, res) => {
  ...
  const result = await session.run(
    'MATCH (u:User {name: $userName})-[r:RATED]->(r:Restaurant)<-[:RATED]-(other:User)-[:RATED]->(rec:Restaurant) WHERE NOT (u)-[:RATED]->(rec) RETURN rec.title, AVG(r.rating) as avgRating ORDER BY avgRating DESC LIMIT 5',
    { userName }
  );
